...
- Bundled "Hydra Shield" extension for container management, settings UI, monitoring
...
2. **Hydra Shield Extension** — Stripped ContainerShield. Background manages containers/profiles/seeds. Inject script calls `window.setXxx()` with domain-specific seeds. JS spoofers handle vectors C++ doesn't cover.
3. **Config** — `policies.json` + `hydra.cfg` restore daily-driver features (search, bookmarks, passwords) while keeping privacy hardened.
...
make extension  # Build Hydra Shield extension
...
- `patches/hydra/` — New patches for daily-driver restoration
- `config/` — policies.json, hydra.cfg, local-settings.js
- `extension/` — Hydra Shield extension (TypeScript, React, Tailwind)
...
- `extension/src/inject/` — Camoufox bridge (calls window.setXxx()), fingerprint monitor
...
- `window.__HYDRA__` config MUST be deleted after reading to prevent page access
