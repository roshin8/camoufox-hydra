...
interface ConfigDeps {
  ...
  getAssignedProfile(cookieStoreId: string): Promise<HydraConfig['profile']>;
  getSettingsForDomain(
    cookieStoreId: string,
    domain: string
  ): Promise<HydraConfig['settings']>;
}
...
export class ConfigInjector {
  ...
  private async injectConfig(
    ...
    try {
      ...
      const config: HydraConfig = { seed, domain, profile, settings };

      await browser.scripting.executeScript({
        target: { tabId, frameIds: [frameId] },
        world: 'MAIN',
        injectImmediately: true,
        func: (cfg: HydraConfig) => {
          (window as Record<string, unknown>).__HYDRA__ = cfg;
        },
        args: [config],
      });
    } catch {
