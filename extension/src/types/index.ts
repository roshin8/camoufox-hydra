...
/** Config injected into MAIN world via window.__HYDRA__ */
...
export type HydraMessage =
  | { type: 'HYDRA_ACTIVE'; domain: string }
  | { type: 'HYDRA_FINGERPRINT_ACCESS'; category: string; api: string }
  | { type: 'HYDRA_GET_STATUS'; tabId: number }
  | { type: 'HYDRA_STATUS'; active: boolean; domain: string; containerId: string };

/**
 * Camoufox self-destructing window methods.
 ...
 */
export interface CamoufoxWindow extends Window {
