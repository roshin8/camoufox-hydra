/**
 * Camoufox Bridge — calls window.setXxx() self-destructing methods
...
import type { AssignedProfile, CamoufoxWindow, SpooferSettings } from '@/types';
...
function callIfAvailable<K extends keyof CamoufoxWindow>(
  method: K,
  ...args: CamoufoxWindow[K] extends ((...a: infer P) => void) | undefined ? P : never[]
): boolean {
  const fn = win[method];
...
/**
 * Configure Camoufox C++ spoofing for the current page.
