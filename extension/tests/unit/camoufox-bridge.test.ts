...
import { configureCamoufoxSpoofing } from '../../src/inject/camoufox-bridge';
...
describe('configureCamoufoxSpoofing', () => {
  beforeEach(() => {
    clearWindowMocks();
  });
  ...
  it('calls all window.setXxx methods when all settings enabled', async () => {
    ...
    await configureCamoufoxSpoofing('seed-123', 'example.com', mockProfile, allEnabledSettings);
  ...
  it('different domains = different seeds', async () => {
    ...
    clearWindowMocks();
    const mocks2 = setupWindowMocks();
    await configureCamoufoxSpoofing('seed-123', 'other.com', mockProfile, allEnabledSettings);
    const canvasSeed2 = mocks2['setCanvasSeed']!.mock.calls[0]![0];
  ...
  it('different containers = different seeds', async () => {
    const mocks1 = setupWindowMocks();
    await configureCamoufoxSpoofing('container-1-seed', 'example.com', mockProfile, allEnabledSettings);
    const canvasSeed1 = mocks1['setCanvasSeed']!.mock.calls[0]![0];

    clearWindowMocks();
    const mocks2 = setupWindowMocks();
    await configureCamoufoxSpoofing('container-2-seed', 'example.com', mockProfile, allEnabledSettings);
    const canvasSeed2 = mocks2['setCanvasSeed']!.mock.calls[0]![0];
  ...
  it('does not call setCanvasSeed when canvas is off', async () => {
    ...
    await configureCamoufoxSpoofing('seed-123', 'example.com', mockProfile, settings);
  ...
  it('gracefully handles missing window.setXxx methods', async () => {
    ...
    await expect(
      configureCamoufoxSpoofing('seed-123', 'example.com', mockProfile, allEnabledSettings)
