...
# then applies all patches in sorted order (same as Camoufox's patch.py).
#
...
# Copy additions (like Camoufox's setup-minimal)
...
for p in $(find "$REPO_DIR/patches" -maxdepth 1 -name '*.patch' | sort); do
    name=$(basename "$p")
    echo -n "  $name ... "
    result=$(patch -p1 -i "$p" 2>&1)
    if echo "$result" | grep -q "malformed\|FAILED"; then
        echo "FAILED"
